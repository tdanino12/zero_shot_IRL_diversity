    ...
    for i, run_id in enumerate(run_ids):
        ...
        if run.state == "finished":
            ...
            for policy_id in range(1, run.config["population_size"] + 1):
                ...
                run2 = wandb.init(project="Overcooked", name=run_id + "_" + str(i))  # this creates a new run
                this_run_id = run2.id
                #artifact = wandb.Artifact("mep1-checkpoints", type="model")
                if ("h7cv3m1b" in run_id):
                    folder = f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191155-h7cv3m1b/files/mep{policy_id}"
                    # For each .pt file, log it under a “mep1/” prefix
                    wandb.save(os.path.join(folder, "**"))  # Double-asterisk for recursion
                    #for filepath in glob.glob(os.path.join(folder, "*.pt")):
                    #    fname = os.path.basename(filepath)
                    #    wandb.log({ f"mep{policy_id}/{fname}": wandb.File(filepath) })
                    #artifact.add_dir(f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191155-h7cv3m1b/files/mep{policy_id}", name=f"mep{policy_id}")
                if ("ykww0g6z" in run_id):
                    folder = f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191325-ykww0g6z/files/mep{policy_id}"
                    wandb.save(os.path.join(folder, "**"))  # Double-asterisk for recursion
                    #artifact.add_dir(f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191325-ykww0g6z/files/mep{policy_id}", name=f"mep{policy_id}")
                if ("u2nzzcpz" in run_id):
                    folder = f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191315-u2nzzcpz/files/mep{policy_id}"
                    wandb.save(os.path.join(folder, "**"))  # Double-asterisk for recursion
                    #artifact.add_dir(f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191315-u2nzzcpz/files/mep{policy_id}", name=f"mep{policy_id}")
                if ("0n8v2sh1" in run_id):
                    folder = f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191543-0n8v2sh1/files/mep{policy_id}"
                    wandb.save(os.path.join(folder, "**"))  # Double-asterisk for recursion
                    #artifact.add_dir(f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191543-0n8v2sh1/files/mep{policy_id}", name=f"mep{policy_id}")
                if ("v8bnf6gy" in run_id):
                    folder = f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250820_183625-v8bnf6gy/files/mep{policy_id}"
                    wandb.save(os.path.join(folder, "**"))  # Double-asterisk for recursion
                    #artifact.add_dir(f"/home/tom.danino/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191543-0n8v2sh1/files/mep{policy_id}", name=f"mep{policy_id}")


                #artifact.add_dir(f"~/ZSC/results/Overcooked/random0/mep/mep-S1-s4-diversity/wandb/run-20250621_191543-{run_id}/files/mep{policy_id}", name=policy_id)
                ...
                print(actor_versions,actor_pts)
